...
export function buildReport(): string {
  ...
  return `# Soft Mutation in Colloquial Welsh: A Predictive Account

*Generated by \`report/report.ts\`. Do not edit by hand: every tree below is
rendered by the program and every stated verdict is asserted at build time —
this document cannot disagree with the implementation that backs it.*
...
issue. Word-internal mutation under derivational prefixes is out of scope: the
predicate ranges over tokens, and prefixed forms are radical lexemes in their
own right.
...
The program therefore implements three subsystems plus a veto layer, each
returning its own provenance:
...
strongest cross-cutting invariant in the literature and it is an invariant of
the program: the environment a word is judged in is a fixed-size record about
one preceding position plus the word's own resolved features, never a whole
tree.
${S(2, 'Reading the diagrams')}
Each example is rendered from the program's tree representation. Child indices
appear on the connectors; every leaf is annotated with the program's verdict
and its provenance. \`⟨N f sg⟩\` gives category and features; \`lemma=\` marks a
trigger-lexicon key that differs from the citation form (homographs like the
two *yn*, the two *ei*); \`gap:NP\` is an extraction site. In the Welsh text,
\`°\` marks a soft-mutated word. Verdict annotations appear on *every* leaf, so
incidental facts (the colloquial mutation of the clause-initial verb, vetoes on
function words) are visible alongside the example's point.
...
are stored as non-triggers, not omitted). AM and NM frames matter to an SM
predicate because they *pre-empt*: a word governed by an AM trigger does not
soft-mutate. Blocking (King §5d) requires no rule — a trigger acts only on the
immediately following word, so an intervener simply *is* the preceding element.
...
category-sensitive, not phonological (Green 2003). Agreement is implemented as
a feature borne by the target (Breit 2019), not as adjacency to the noun: that
...
despite string adjacency (Mittendorf & Sadler 2006; Dowle 2024) — in the
program this is the *absence of a licensing relation*, derived from the
genitive configuration, not a veto.
...
a phrase intervenes (Green 2003) — facts the XPTH derives for free. The program
implements the XPTH geometrically: a phrasal node (never a clause) whose right
edge immediately precedes the target and c-commands it licenses \`synt:xp-edge\`.
Extraction gaps count as phrases (Willis 2007 documents the transparency
lineage); following Dowle (2024) we posit no other empty categories.

Two implementation findings are worth recording as results. First, raw XPTH
geometry overgenerates **NP-internally**: in a masculine chain like *ci mawr
coch*, AP(*mawr*) ends immediately before *coch* and c-commands it, yet *coch*
is radical. The program therefore excludes candidates whose join node with the
target is an NP — Tallerman's 'complement' condition operationalized, and
exactly Breit's (2019) division of labor: NP-internal mutation belongs to the
gender subsystem alone. Second, clause-initial finite-verb mutation
(\`synt:v1-*\`) is **not XPTH-derivable even in principle**: nothing precedes a
clause-initial verb, and the negative variant surfaces as *mixed* mutation
(AM on *c/p/t*) where the XPTH only ever produces SM. It is particle-drop
residue — affirmative *fe°/mi°*, interrogative *a°*, negative *ni* (mixed) were
deleted, and their mutations remained (King §481 note; Green 2003 on *ni*).
The grade tracks the vanished particle exactly. In formal literary Welsh the
affirmative default is particle-less and radical, so this entire rule is
register-specific (King §11d) — encoded as designated positions, it is a data
toggle, not a theory change. Imperatives resist both v1 mutation and *neu°*
(King §512), which the category system encodes directly.
...
The two are distinguished in provenance because they are different theoretical
claims: one is lexical listing, the other phonological vacuity.

${examples(SECTIONS[3]!.examples)}
${S(7, 'Contested territory')}
A predictive account must say where prescription itself is unstable. These are
flagged in the program's contested registry; test failures in these contexts
are reported as *contested*, not wrong. Corpus context: overall prescriptive
...
Every contact frame in the program, with its King reference. A lemma may carry
several frames (*y*, *chwe*); \`none\` frames record deliberate non-triggers.

${triggerTable()}
${S(9, 'Known limits')}
- **Adverbial and vocative status are author-supplied.** Geometry provably
  underdetermines them: sentence-initial adverbial NPs mutate while
  geometrically identical fronted objects do not. The distinction is
  adjunct-vs-argument, a valence/lexical fact — every formal account consumes
  it as input (Dowle 2024 takes it from f-structure), none derives it.
- **Common-noun apposition** shares the genitive configuration and would be
  misclassified as a possessor; appositive NPs are overwhelmingly personal
  names (immutable), so the exposure is minimal.
...
- **Usage variability** (§7) is out of predictive scope by design; a usage
  model would attach per-trigger application rates to the same rule inventory.
- The mixed-mutation grade of the negative relative particle *na* is encoded
  from prescriptive tradition; King's own examples (§481) are grade-ambiguous
